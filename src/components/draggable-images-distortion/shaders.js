...
export const deformationFragmentShader = `
  ...
  vec2 getDeformedImageUV(vec2 imageUV) {
    vec2 newUV = imageUV;
    newUV.x -= sin(imageUV.y * PI) * uDelta.x * uAmplitude;
    newUV.y -= sin(imageUV.x * PI) * uDelta.y * uAmplitude;
    return newUV;
  }
  ...
  void main() {
    ...
    vec2 deformedImageUV = getDeformedImageUV(imageUV);

    float edgeSmooth = 0.01;
    vec2 imageMask = smoothstep(-edgeSmooth, edgeSmooth, deformedImageUV) *
                    smoothstep(-edgeSmooth, edgeSmooth, 1.0 - deformedImageUV);
    float imageAlpha = imageMask.x * imageMask.y;

    bool inImageArea = deformedImageUV.x >= 0.0 && deformedImageUV.x <= 1.0 && deformedImageUV.y >= 0.0 && deformedImageUV.y <= 1.0;
